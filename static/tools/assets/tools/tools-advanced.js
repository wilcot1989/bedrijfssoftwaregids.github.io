  ...
  if (toolType === "zzp_bv") {
    ...
    qs("#run").addEventListener("click", () => {
      ...
      qs("#out").innerHTML = `<div class="bsg-result">
        ...
        <p style="font-size:1.1rem;font-weight:600;color:#111827;margin:0.75rem 0 0.25rem;">${escapeHTML(verdict)}</p>
  ...
  if (toolType === "agile_score") {
    ...
    qs("#run").addEventListener("click", () => {
      ...
      qs("#out").innerHTML = `<div class="bsg-result">
        ...
        <div style="background:#e5e7eb;border-radius:6px;height:10px;margin:0.75rem 0 0.5rem;overflow:hidden;">
          <div style="background:#059669;height:100%;width:${pct}%;border-radius:6px;transition:width 0.3s;"></div>
        </div>
        <p style="font-size:1.05rem;font-weight:600;color:#111827;margin:0;">${escapeHTML(level)}</p>
      </div>`;
      renderKeuzehulpCTA(catKey);
    });
    return;
